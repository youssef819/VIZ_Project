...
function getTeamsByOutcome(outcome) {
    ...
    manCityMatches.forEach(match => {
        if (outcome === 'Matchs gagnés') {
            if (match['3'] === Team && match['6'] > match['7']) {
                teams.push(match['4']);
            } else if (match['4'] === Team && match['7'] > match['6']) {
                teams.push(match['3']);
            }
        } else if (outcome === 'Matchs perdus') {
            if (match['3'] === Team && match['6'] < match['7']) {
                teams.push(match['4']);
            } else if (match['4'] === Team && match['7'] < match['6']) {
                teams.push(match['3']);
            }
        } else if (outcome === 'Matchs nuls') {
            if (match['6'] === match['7']) {
                if (match['3'] === Team) {
                    teams.push(match['4']);
...
const pieChartData = [
    { label: 'Matchs nuls', value: draws },
    { label: 'Matchs gagnés', value: wins },
    { label: 'Matchs perdus', value: losses }
];
...
arcs.append('image')
    .attr('xlink:href', d => {
        // Insérez les liens vers les images correspondant à chaque catégorie (matchs nuls, gagnés et perdus)
        if (d.data.label === 'Matchs nuls') {
            return '';
        } else if (d.data.label === 'Matchs gagnés') {
            return '';
        } else if (d.data.label === 'Matchs perdus') {
            return '';
        }
    })
    .attr('width', 421)
    .attr('height', 402)
    .attr('mask', (_, i) => `url(#mask-${i})`)
    .attr('transform', d => `translate(${arc.centroid(d)})`)
    .style('pointer-events', 'none'); // Assurez-vous que les images ne captent pas les événements de souris pour que les événements de survol fonctionnent correctement
...
function piechart2(matches, Season, Team,container,tooltip){
  ...
  const radius = Math.min(width, height) / 2;
  const color = d3.scaleOrdinal(d3.schemeCategory10);
  const margin = ({top: 0, right: 0 , bottom: 0, left: 450})
